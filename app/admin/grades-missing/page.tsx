...
import {
  ChevronLeft, ClipboardList, Search, RefreshCw, Loader2, ArrowRight,
} from 'lucide-react';
import { toast } from 'sonner';
import { Student } from '@/lib/types/student';
import { isWeeklyGradeMissing, weekStartYmd } from '@/lib/student-flags';
...
export default function GradesMissingPage() {
  ...
  const campusScopedStudents = students.filter(s =>
    campusFilter === 'all' || s.campus === campusFilter
  );

  const missingStudents = campusScopedStudents
    .filter(s => isWeeklyGradeMissing(s))
    .sort((a, b) => a.name.localeCompare(b.name, 'ko'));

  const filtered = missingStudents.filter(s =>
    !searchQuery ||
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] text-[#1D1D1F] font-sans">
      <AdminTopNav title="성적 미입력 원생 목록" onLogout={handleLogout} />
      ...
      <main className="mx-auto max-w-4xl px-4 pt-6 pb-16 sm:px-6 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 mt-0.5">
                이번 주 ({weekStart} 이후) 성적 미입력 · 매주 성적 입력 대상 원생만 표시합니다.
              </p>
            </div>
          </div>
          <Button
        ...
        <div className={`rounded-2xl border px-5 py-4 flex items-center gap-4 ${
          ...
          <ClipboardList className={`w-5 h-5 shrink-0 ${filtered.length > 0 ? 'text-amber-500' : 'text-slate-300'}`} />
          <div>
            <p className={`text-sm font-black ${filtered.length > 0 ? 'text-amber-800' : 'text-slate-400'}`}>
              {filtered.length > 0
                ? `이번 주 성적 미입력 원생 ${filtered.length}명`
                : '이번 주 성적을 모두 입력했습니다 🟢'}
            </p>
            {filtered.length > 0 && (
              <p className="text-[11px] font-semibold text-amber-600 mt-0.5">
                원생 클릭 시 상세 시트에서 성적 입력 가능
              </p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-100 bg-white shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="py-20 text-center text-sm font-bold text-slate-400">
              {searchQuery ? '검색 결과가 없습니다.' : '이번 주 성적 미입력 원생이 없습니다 🟢'}
            </div>
          ) : (
              ...
              <table className="w-full border-collapse text-left text-xs font-semibold text-slate-600">
                <thead className="bg-slate-50/80 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                  <tr>
                    ...
                    <th className="px-6 py-4">마지막 성적 입력</th>
                    <th className="px-6 py-4 text-right">상세 보기</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100/60">
                  {filtered.map(student => {
                    const sortedGrades = [...(student.grades || [])].sort((a, b) => b.date.localeCompare(a.date));
                        ...
                        <td className="px-6 py-4">
                          {lastGrade ? (
                            <span className="text-amber-600 font-bold">
                              {lastGrade.date} · {lastGrade.subject} {lastGrade.score}점
                            </span>
                          ) : (
                            <span className="text-slate-400 font-bold">성적 기록 없음</span>
                          )}
                        </td>
