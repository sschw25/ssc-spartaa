...
import { Clock, ChevronLeft, Search, Loader2, ArrowRight } from 'lucide-react';
...
export default function EnrollmentWarningPage() {
  ...
  return (
    <div className="ios-app-bg min-h-screen text-slate-900 dark:text-slate-100 font-sans">
      <AdminTopNav title="재등록 임박 학생 목록" onLogout={handleLogout} actions={<AdminNavActions onRefresh={loadStudents} loading={loading} onLogout={handleLogout} />} />
      ...
      <main className="mx-auto max-w-5xl px-4 pt-6 pb-16 sm:px-6 space-y-6">
        ...
        <div className={`rounded-2xl border px-5 py-4 flex items-center gap-4 ${
          filtered.length > 0 ? 'bg-amber-50 dark:bg-amber-500/10 border-amber-200/70 dark:border-white/10' : 'bg-white dark:bg-[#1c1c1e] border-slate-100 dark:border-white/10'
        }`}>
          <Clock className={`w-5 h-5 shrink-0 ${filtered.length > 0 ? 'text-amber-500' : 'text-slate-300 dark:text-slate-600'}`} />
          <div>
            <p className={`text-sm font-black ${filtered.length > 0 ? 'text-amber-800 dark:text-amber-300' : 'text-slate-400'}`}>
              {filtered.length > 0
                  ...
                  {filtered.map(({ student, daysLeft }) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <ArrowRight className="w-4 h-4 ml-auto text-slate-300 dark:text-slate-600 group-hover:text-amber-500 transition-colors" />
                      </td>
                    </tr>
                  ))}
                </tbody>
