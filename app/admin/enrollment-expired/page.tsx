...
export default function EnrollmentExpiredPage() {
  ...
  const expiredStudents = campusScopedStudents
    .filter(s => {
      const d = enrollmentDaysLeft(s.enrollmentEndDate);
      return d !== null && d < 0;
    })
    .map(s => ({ student: s, daysLeft: enrollmentDaysLeft(s.enrollmentEndDate)! }))
    .sort((a, b) => a.daysLeft - b.daysLeft); // 가장 오래된 만료 순

  const filtered = expiredStudents.filter(({ student }) =>
    !searchQuery ||
