  ...
  return (
    <div className="admin-fluid-ui ios-app-bg min-h-screen text-slate-900 font-sans">
      <AdminTopNav
        ...
        actions={
          <Button size="sm" variant="outline" onClick={loadAll} className="rounded-2xl border-black/[0.05] text-xs h-9.5 bg-white px-3 shadow-sm" title="새로고침">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="bg-white rounded-3xl border border-black/[0.05] shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-black/[0.04] bg-[#FAFAFA] flex items-center gap-2">
              ...
              <button onClick={openEventModal} className="flex items-center gap-1.5 text-[11px] font-extrabold text-[#0071E3] bg-[#0071E3]/[0.07] hover:bg-[#0071E3]/[0.12] rounded-xl px-3 py-1.5 transition-colors shrink-0">
                <Plus className="w-3.5 h-3.5" /> 일정
              </button>
            </div>

            <div className="divide-y divide-black/[0.03] max-h-[720px] overflow-y-auto">
              {totalSelected === 0 && (
              ...
              {selectedCampusEvents.length > 0 && (
                <div className="pb-3">
                  <SectionHeader color="bg-[#0071E3]" icon={<CalendarHeart className="w-3.5 h-3.5 text-[#0071E3]" />} title="일정 · 참여 미션" count={selectedCampusEvents.length} chip="bg-[#0071E3]/10 text-[#0071E3] border-[#0071E3]/20" />
                  <div className="px-3 pt-2 space-y-2">
                    {selectedCampusEvents.map((ev) => {
                      ...
                      return (
                        <div key={ev.id} className={`rounded-xl border overflow-hidden bg-white shadow-sm ${ev.isMission ? 'border-l-[3px] border-l-[#0071E3] border-black/[0.05]' : 'border-l-[3px] border-l-slate-400 border-black/[0.05]'}`}>
                          <div className="px-3.5 py-2.5 space-y-1.5">
                            <div className="flex items-start gap-2">
                              ...
                              <button onClick={() => deleteEvent(ev.id)} className="rounded-lg p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 transition shrink-0" title="삭제">
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
                            </div>
                            {ev.memo && <p className="text-[11px] font-semibold text-slate-600 leading-relaxed whitespace-pre-wrap break-words rounded-lg bg-slate-50 px-2.5 py-2">{ev.memo}</p>}
                              ...
                              <>
                                ...
                                <div className="flex items-center gap-1.5 pt-1">
                                  ...
                                  <button disabled={!!busy[`grant_${ev.id}`] || !ev.notifiedAt} onClick={() => grantEventCoupons(ev)}
                                    ...
                                    className={`flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-[11px] font-black transition active:scale-95 ${ev.rewardedAt ? 'bg-amber-50 text-amber-600 border border-amber-200' : 'bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-40'}`}>
                                  ...
                                  <button onClick={() => { setExpandedMissionId(isMissionOpen ? null : ev.id); setMissionSearch(''); }}
                                    className="ml-auto flex items-center gap-1 rounded-lg px-2 py-1.5 text-[11px] font-bold text-slate-500 hover:bg-slate-50">
                                    참여자 <ChevronDown className={`w-3.5 h-3.5 transition-transform ${isMissionOpen ? 'rotate-180' : ''}`} />
                                  </button>
                                </div>

                                {isMissionOpen && (
                                  <div className="rounded-xl border border-black/[0.05] bg-[#FAFAFA] p-2 space-y-1.5 mt-1">
                                    <div className="relative">
                                      <input value={missionSearch} onChange={(e) => setMissionSearch(e.target.value)} placeholder="학생 검색"
                                        className="w-full rounded-lg border border-black/[0.08] bg-white pl-7 pr-3 py-1.5 text-[11px] font-semibold focus:border-[#0071E3] focus:outline-none" />
                                      <Search className="w-3 h-3 text-slate-400 absolute left-2.5 top-1/2 -translate-y-1/2" />
                                    </div>
                                    <div className="max-h-52 overflow-y-auto space-y-1">
                                      {targetList.length === 0 && <p className="text-[11px] text-slate-400 font-semibold py-2 text-center">대상 학생이 없습니다.</p>}
                                      {targetList.map((s) => {
                                        const p = (s.eventParticipations || []).find((x) => x.eventId === ev.id);
                                        const pk = `part_${s.id}_${ev.id}`;
                                        return (
                                          <div key={s.id} className="flex items-center gap-2 rounded-lg bg-white border border-black/[0.04] px-2.5 py-1.5">
                                            <span className="text-[11px] font-bold text-slate-700 flex-1 min-w-0 truncate">{s.name}
                                              <span className="text-[9px] text-slate-400 ml-1">{campusLabel(s.campus)}</span>
                                              {p?.rewarded && <span className="ml-1 inline-flex items-center gap-0.5 rounded bg-amber-50 text-amber-600 px-1 py-0.5 text-[9px] font-black"><Ticket className="w-2 h-2" />지급</span>}
                                            </span>
                                            <button disabled={!!busy[pk]} onClick={() => setParticipation(s.id, ev.id, 'accepted')}
                                              className={`rounded-md px-2 py-1 text-[10px] font-black transition ${p?.status === 'accepted' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-400 border border-slate-200 hover:border-emerald-300'}`}>수락</button>
                                            <button disabled={!!busy[pk]} onClick={() => setParticipation(s.id, ev.id, 'declined')}
                                              className={`rounded-md px-2 py-1 text-[10px] font-black transition ${p?.status === 'declined' ? 'bg-slate-500 text-white' : 'bg-white text-slate-400 border border-slate-200 hover:border-slate-300'}`}>불참</button>
                                          </div>
                                        );
                                      })}
                                    </div>
                                    ...
                                    {ev.request.status === 'pending' && (
                                      <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                          ...
                                          <Button size="sm" variant="outline" disabled={busy[revKey]} onClick={() => reviewRequest(ev, 'rejected')} className="h-9 rounded-xl border-red-200 text-red-600 hover:bg-red-50 text-xs font-bold"><X className="w-3.5 h-3.5 mr-1" /> 반려</Button>
                                        </div>
                                      </div>
                                    )}
              ...
              {selectedDateConsultations.length > 0 && (
                <div className="py-4">
                  <SectionHeader color="bg-emerald-500" icon={<MessageSquare className="w-3.5 h-3.5 text-emerald-600" />} title="당일 상담 진행" count={selectedDateConsultations.length} chip="bg-emerald-50 text-emerald-700 border-emerald-200/60" />
                  <div className="px-3 pt-2 space-y-2">
              ...
              {selectedDatePenalties.length > 0 && (
                <div className="py-4">
                  <SectionHeader color="bg-amber-500" icon={<ShieldAlert className="w-3.5 h-3.5 text-amber-600" />} title="당일 벌점·상점" count={selectedDatePenalties.length} chip="bg-amber-50 text-amber-700 border-amber-200/60" />
                  <div className="px-3 pt-2 space-y-2">
                ...
                <div className="space-y-3 pt-1">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button size="icon" variant="outline" onClick={() => setEvForm((f) => ({ ...f, couponReward: f.couponReward + 1 }))} className="h-7 w-7 rounded-lg border-black/[0.08]"><Plus className="w-3 h-3" /></Button>
                  ...
                  {evForm.targetMode === 'students' && (
                    <div className="space-y-1.5">
                      ...
                      <div className="max-h-44 overflow-y-auto rounded-xl border border-black/[0.06] divide-y divide-black/[0.03]">
                        {[...students]
