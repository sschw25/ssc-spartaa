...
import { approvePendingStudentPasswordChange, getStudentById, updateStudentById } from '@/lib/store';
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await approvePendingStudentPasswordChange(id);
    if (result === 'not_found' || result === 'no_pending') {
    ...
    return NextResponse.json({ success: true, message: `${result.name} 학생의 출결번호 변경을 승인했습니다.` });
  } catch (error) {
