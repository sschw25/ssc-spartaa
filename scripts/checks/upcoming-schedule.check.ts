...
// 알림 선택 발송(recipientStudentIds) — 정의되면 campus/targetMode 폴백보다 우선
assert(isCampusEventTargetedToStudent(makeEvent({ recipientStudentIds: ['s1'] }), student), '행사 수신자 포함 → 대상');
assert(!isCampusEventTargetedToStudent(makeEvent({ recipientStudentIds: ['s2'] }), student), '행사 수신자 미포함 → 비대상');
assert(!isCampusEventTargetedToStudent(makeEvent({ campus: 'wonju', recipientStudentIds: ['s2'] }), student), '행사 수신자 지정 시 센터일치라도 비대상(수신자 우선)');
