    ...
    <div id="timetable" className={`scroll-mt-24 space-y-5 print-card ${!isStudentReport || activeTab === 'timetable' ? '' : 'hidden print:block'}`}>
      ...
      {isStudentReport && (
        <div className="rounded-3xl border border-[#0071E3]/15 dark:border-white/10 bg-white dark:bg-[#1c1c1e] p-5 md:p-6 shadow-sm space-y-4 print-card">
          ...
          <div className="relative border-l border-slate-100 dark:border-white/10 pl-4.5 ml-2.5 space-y-3.5 my-2">
                  ...
                  <div className={`p-3 rounded-2xl border transition-all text-left ${
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-1.5">
                      <div className="flex items-center gap-1.5">
                        {periodNumLabel ? (
                          <Badge className="bg-slate-100 dark:bg-white/10 hover:bg-slate-100 text-slate-700 dark:text-slate-300 text-[10px] font-black shrink-0 px-2 py-0.5 rounded-lg border-0">
                            {periodNumLabel}
                          </Badge>
                        ) : (
                          <Coffee className="w-3.5 h-3.5 shrink-0 text-slate-400 dark:text-slate-400" />
                        )}
                        <span className="text-[11px] font-black text-slate-800 dark:text-slate-200">{period.label.split(':')[1]?.trim() || period.label}</span>
                      </div>
