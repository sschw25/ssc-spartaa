  ...
  return (
    <div className="@container rounded-3xl border border-black/[0.05] bg-white p-6 md:p-8 shadow-sm space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-[15px] font-semibold text-slate-900">순공 시간</h3>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 @md:grid-cols-3 gap-3">
            <div className="rounded-2xl bg-[#F5F5F7] p-4">
              ...
              <div className="text-lg @md:text-xl font-bold text-slate-900 mt-1 tabular-nums whitespace-nowrap">{fmt(stats.weekTotalMin)}</div>
            </div>
            <div className="rounded-2xl bg-[#F5F5F7] p-4">
              <div className="text-[11px] text-slate-500 font-semibold flex items-center gap-1">
                <CalendarDays className="w-3.5 h-3.5 shrink-0" /> 이번 달 순공
              </div>
              <div className="text-lg @md:text-xl font-bold text-slate-900 mt-1 tabular-nums whitespace-nowrap">{fmt(stats.monthTotalMin)}</div>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-slate-900">요일별 공부량 (이번 달)</span>
              {stats.peakWeekday && (
                ...
                return (
                  <div key={d.label} className="flex-1 flex flex-col items-center justify-end h-full gap-1">
                    <div
                      ...
                      title={fmt(d.min)}
                    />
                    <span className={`text-[10px] ${isPeak ? 'text-[#F56300] font-bold' : 'text-slate-500'}`}>{d.label}</span>
                  </div>
                );
              })}
            </div>
