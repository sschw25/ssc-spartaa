...
const REDEMPTION_STATUS: Record<Redemption['status'], { label: string; cls: string }> = {
  requested: { label: '승인 대기', cls: 'bg-amber-100 dark:bg-amber-500/10 text-amber-700' },
  pending: { label: '지급 대기', cls: 'bg-[#0071E3]/10 dark:bg-[#0071E3]/15 text-[#0071E3]' },
  fulfilled: { label: '지급 완료', cls: 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' },
  rejected: { label: '반려', cls: 'bg-slate-200 dark:bg-white/10 text-slate-500' },
};
...
export function CouponExchangeCard({ onCouponsChange }: { onCouponsChange?: (coupons: number) => void }) {
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-black/5 dark:border-white/10 bg-white dark:bg-[#1c1c1e] p-6 shadow-sm flex items-center justify-center">
  ...
  return (
    <div className="mx-auto w-full max-w-[680px] px-4 sm:px-5 space-y-4">
      ...
      <div className="rounded-3xl border border-[#0071E3]/15 bg-[#0071E3]/[0.03] dark:bg-[#0071E3]/15 p-5 shadow-sm md:p-6">
        ...
        <h3 className="mt-2 text-xl font-black text-slate-900 dark:text-slate-100">쿠폰 교환소</h3>
        <p className="mt-1 flex flex-wrap items-center gap-x-2 text-[11px] font-semibold leading-5 text-slate-500 dark:text-slate-400">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white dark:bg-[#1c1c1e] border border-[#0071E3]/20 px-2.5 py-1 text-xs font-black text-[#0071E3] shadow-sm">
      ...
      {/* 교환 카탈로그 */}
      <div className="rounded-xl border border-black/5 dark:border-white/10 bg-white dark:bg-[#1c1c1e] p-5 shadow-sm sm:p-6 space-y-3">
            ...
            {data.rewardCatalog.map((r) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-between rounded-lg border px-3 py-2.5 text-left transition active:scale-[0.97] ${affordable ? 'border-slate-200 dark:border-white/10 bg-white dark:bg-[#1c1c1e] hover:border-[#0071E3]/40' : 'border-slate-100 dark:border-white/10 bg-slate-50 dark:bg-white/5 opacity-60'}`}
                >
                  <span className="min-w-0">
                    <span className="block text-xs font-semibold text-slate-700 dark:text-slate-300">{r.label}</span>
      ...
      {activeRedemptions.length > 0 && (
        <div className="rounded-xl border border-black/5 dark:border-white/10 bg-white dark:bg-[#1c1c1e] p-5 shadow-sm sm:p-6 space-y-2">
          ...
          {activeRedemptions.map((r) => {
            ...
            return (
              <div key={r.id} className="flex items-center gap-2 rounded-lg border border-slate-100 dark:border-white/10 bg-white dark:bg-[#1c1c1e] px-3 py-2 text-[11px]">
                <span className="font-semibold text-slate-700 dark:text-slate-300">{meta?.label || r.type}</span>
                ...
                {r.status === 'requested' && (
                  <button
                    ...
                    className="ml-auto shrink-0 text-slate-300 dark:text-slate-600 transition-colors hover:text-red-500"
                    aria-label="교환 신청 취소"
                  >
      ...
      {doneRedemptions.length > 0 && (
        <div className="rounded-xl border border-black/5 dark:border-white/10 bg-white dark:bg-[#1c1c1e] p-5 shadow-sm sm:p-6 space-y-2">
          ...
          {doneRedemptions.map((r) => {
            const meta = data.rewardCatalog.find((c) => c.type === r.type);
            return (
              <div key={r.id} className="rounded-lg border border-emerald-200 dark:border-emerald-500/20 bg-emerald-50/50 dark:bg-emerald-500/10 px-3 py-2.5 text-[11px]">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                  <span className="font-semibold text-slate-800 dark:text-slate-200">{meta?.label || r.type}</span>
                  {r.fulfilledAt && <span className="text-[10px] font-semibold text-slate-400">{r.fulfilledAt.slice(0, 10)}</span>}
                </div>
                {r.voucherCode && (
                  <div className="mt-1.5 flex items-center gap-1.5 rounded-lg border border-emerald-200 dark:border-emerald-500/20 bg-white dark:bg-[#1c1c1e] px-2.5 py-1.5">
                    <Sparkles className="w-3 h-3 text-emerald-500 shrink-0" />
                    <span className="text-[10px] font-semibold text-slate-500 dark:text-slate-400">교환 번호</span>
                    <span className="ml-auto select-all font-mono text-xs font-black tracking-wider text-slate-900 dark:text-slate-100">{r.voucherCode}</span>
                  </div>
                )}
                {r.note && <p className="mt-1 font-semibold text-slate-500 dark:text-slate-400">{r.note}</p>}
              </div>
            );
          })}
        </div>
