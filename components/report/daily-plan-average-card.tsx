...
export function DailyPlanAverageCard() {
  ...
  // 오늘 계획을 가진 등원 학생이 없으면 숨김
  if (!data.studentCount) return null;
    ...
    <div className="rounded-3xl border border-black/[0.05] dark:border-white/10 bg-white dark:bg-[#1c1c1e] p-6 md:p-8 shadow-sm space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[15px] font-semibold text-slate-900 dark:text-slate-100 break-keep">오늘 등원 학생 평균 달성도</h3>
        </div>
        <span className="shrink-0 inline-flex items-center gap-1.5 text-[11px] font-bold text-[#0071E3] bg-[#0071E3]/[0.06] dark:bg-[#0071E3]/15 border border-[#0071E3]/15 px-2.5 py-1 rounded-full">
          <TrendingUp className="w-3.5 h-3.5" /> {data.studentCount}명 평균
        </span>
      </div>
      ...
      <div className="rounded-2xl bg-[#0071E3]/[0.05] dark:bg-[#0071E3]/15 border border-[#0071E3]/15 p-5">
        <div className="flex items-end justify-between gap-3">
          <div>
            <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400">오늘 계획 달성률(평균)</p>
      ...
      <p className="text-[10px] text-slate-500 dark:text-slate-400 text-center break-keep">오늘 등원한 학생들의 <b>오늘 계획</b> 달성도 평균이에요. 개인 정보는 보이지 않아요.</p>
    </div>
  );
}
