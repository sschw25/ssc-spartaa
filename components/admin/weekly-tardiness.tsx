...
function Th({ k, label, className = '', sortKey, sortDir, onSort }: { k: SortKey; label: string; className?: string; sortKey: SortKey; sortDir: SortDir; onSort: (k: SortKey) => void }) {
  return (
    <th className={`px-4 py-3 ${className}`}>
      <button onClick={() => onSort(k)} className="inline-flex items-center gap-1 font-bold text-slate-900 dark:text-slate-100 hover:text-[#0071E3]">{label} <SortIcon active={sortKey === k} dir={sortDir} /></button>
    </th>
  );
}
...
export function WeeklyTardiness({ campusFilter }: { campusFilter: string }) {
  ...
  if (loading) return <div className="flex items-center justify-center py-16 bg-white dark:bg-[#1c1c1e] rounded-2xl border border-black/[0.05] dark:border-white/10"><Loader2 className="w-6 h-6 text-[#0071E3] animate-spin mr-2" /><span className="text-xs text-slate-500 dark:text-slate-400">불러오는 중…</span></div>;
  if (error) return <div className="py-16 text-center text-sm text-red-600 font-semibold bg-white dark:bg-[#1c1c1e] rounded-2xl border border-black/[0.05] dark:border-white/10">{error}</div>;

  return (
    <div className="space-y-3">
      {data?.summary && (
        <div className="flex flex-wrap gap-3 text-xs font-bold">
          <span className="px-3 py-1.5 rounded-full bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-300 border border-red-100 dark:border-red-500/20">이번 주 지각 학생 {data.summary.lateStudents}명</span>
          ...
          <span className="px-3 py-1.5 rounded-full bg-[#F5F5F7] dark:bg-white/5 text-slate-500 dark:text-slate-400">기준: {data?.weekStart} ~ {data?.today}</span>
        </div>
      )}
      <div className="bg-white dark:bg-[#1c1c1e] border border-black/[0.05] dark:border-white/10 rounded-2xl shadow-sm overflow-x-auto">
        {rows.length === 0 ? (
          <div className="py-16 text-center text-sm text-slate-500 dark:text-slate-400">이번 주 출결 기록이 없습니다.</div>
        ) : (
          <table className="w-full text-xs">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className={`border-b border-black/[0.04] dark:border-white/10 hover:bg-[#F5F5F7]/60 dark:hover:bg-white/5 ${r.lateDays >= 3 ? 'bg-red-50/40 dark:bg-red-500/10' : ''}`}>
                  ...
                  <td className="px-4 py-3 font-bold tabular-nums dark:text-slate-200">{r.expectedArrival}</td>
                  <td className="px-4 py-3 tabular-nums dark:text-slate-200">{r.attendedDays}일</td>
                  <td className="px-4 py-3">
                    {r.lateDays > 0
                      ? <span className={`font-bold tabular-nums inline-flex items-center gap-0.5 ${r.lateDays >= 3 ? 'text-red-700 dark:text-red-300' : 'text-[#FF9500]'}`}>{r.lateDays}일{r.lateDays >= 3 && <TriangleAlert className="w-3 h-3 shrink-0" />}</span>
                      : <span className="text-emerald-700 dark:text-emerald-300 font-bold">0일</span>}
                  </td>
                  <td className="px-4 py-3 tabular-nums text-slate-500 dark:text-slate-400">{r.lateRate}%</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <p className="text-[10px] text-slate-500 dark:text-slate-400 text-center">이번 주 학생별 지각 누적(본인 기준 대비). 지각 3일 이상은 <TriangleAlert className="inline w-2.5 h-2.5 -mt-0.5" /> 강조됩니다. 컬럼 머리글로 정렬하세요.</p>
    </div>
  );
}
