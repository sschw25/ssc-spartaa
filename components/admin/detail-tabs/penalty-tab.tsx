    ...
    <div className="space-y-5">
      ...
      <div className={`flex items-center gap-4 rounded-2xl px-5 py-4 border ${
        totalPenalty > 0
          ...
          : 'bg-[#F5F5F7] dark:bg-white/5 border-black/[0.05] dark:border-white/10'
      }`}>
        <Shield className={`w-6 h-6 shrink-0 ${
          totalPenalty > 0 ? 'text-red-500' : totalPenalty < 0 ? 'text-emerald-500' : 'text-slate-400'
        }`} />
        <div>
          <p className="text-xs font-bold text-slate-500 dark:text-slate-400">누적 벌점</p>
      ...
      {/* 등록 폼 */}
      <form onSubmit={handleAdd} className="rounded-2xl border border-black/[0.05] dark:border-white/10 bg-[#F5F5F7] dark:bg-white/5 p-4 space-y-3">
        <p className="text-xs font-semibold text-slate-900 dark:text-slate-100">벌점 · 상점 부여</p>
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-semibold border transition ${
              type === 'penalty'
                ...
                : 'bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:border-red-300'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-semibold border transition ${
              type === 'bonus'
                ...
                : 'bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:border-emerald-300'
            }`}
          >
        ...
        <div className="flex gap-2">
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-3 py-2 text-xs font-semibold text-slate-800 dark:text-slate-200 focus:border-slate-400 focus:outline-none"
            />
          </div>
          <div className="w-24">
      ...
      {/* 내역 */}
      <div className="rounded-2xl border border-black/[0.05] dark:border-white/10 bg-white dark:bg-[#1c1c1e] overflow-hidden">
        <div className="px-4 py-3 border-b border-black/[0.04] dark:border-white/10 bg-[#F5F5F7] dark:bg-white/5">
          <p className="text-xs font-semibold text-slate-900 dark:text-slate-100">내역 ({penalties.length}건)</p>
        </div>
        {penalties.length === 0 ? (
            ...
            {penalties.map((p) => (
              <div key={p.id} className="flex items-center gap-3 px-4 py-3 group">
                <span className={`shrink-0 grid w-10 h-10 place-items-center rounded-xl text-sm font-semibold ${
                  p.type === 'penalty' ? 'bg-red-50 dark:bg-red-500/10 text-red-600' : 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600'
                }`}>
                  {p.type === 'penalty' ? '+' : '-'}{p.points}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-slate-900 dark:text-slate-100 truncate">{p.reason}</p>
                ...
                <button
                  ...
                  className="shrink-0 opacity-0 group-hover:opacity-100 rounded-lg p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition"
                >
                  {deleting === p.id
