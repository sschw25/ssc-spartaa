...
const getProgramsTabs = (location: string) => [
  {
    ...
    title: `${location} 공무원학원 (공단기)`,
    subtitle: '국가직/지방직 맞춤형 압도적 순공시간',
    stat: '자세히 보기',
  },
  {
    id: 'police',
    badge: '2026 체력·필기 통합관리',
    title: `${location} 경찰학원`,
    subtitle: '필기부터 체력·면접까지 직렬 특화 관리',
    stat: '자세히 보기',
  },
  {
    id: 'fire',
    badge: '2026 개편 완벽대비',
    title: `${location} 소방학원`,
    subtitle: '행정법 개편·체력까지 합격 로드맵 관리',
    stat: '자세히 보기',
  },
  {
    id: 'suneung',
    badge: '합리적 프리미엄 독학재수',
    title: `${location} 독학재수학원`,
    ...
    <section id="programs" className="bg-[#F5F5F7] py-20 px-4 sm:px-6 border-t border-black/[0.04]" ref={ref}>
      <div className="max-w-[64rem] mx-auto">
        ...
        <div className="mb-14 text-center fade-in-up">
          <p className="text-[#007AFF] text-[11px] sm:text-xs font-semibold tracking-[0.18em] uppercase mb-4">Select Program</p>
          <h2 className="text-[#1D1D1F] text-4xl sm:text-5xl font-semibold tracking-tight mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {programsTabs.map((prog, index) => (
            ...
            >
              <div>
                <span className="inline-flex px-3 py-1 rounded-full bg-white/10 text-white/90 text-[13px] font-medium tracking-tight mb-6 backdrop-blur-md">
                  {prog.badge}
                </span>

                <h3 className="text-white font-semibold text-3xl sm:text-4xl tracking-tight mb-2">
                  {prog.title}
                </h3>
