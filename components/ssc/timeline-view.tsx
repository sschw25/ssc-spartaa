    ...
    <section id="timeline" className="py-16 md:py-24 bg-[#F5F5F7]" ref={ref}>
      <div className="max-w-[72rem] mx-auto px-6 sm:px-8">
        <div className="mb-12 md:mb-16 text-center fade-in-up">
            <p className="text-[#007AFF] text-[11px] sm:text-xs font-semibold tracking-[0.3em] uppercase mb-6 opacity-80">Daily Workflow</p>
            <h2 className="text-4xl md:text-5xl lg:text-7xl font-semibold text-[#1D1D1F] tracking-tighter mb-8 leading-[1.05] text-balance">
              가장 치열한 몰입,<br />
              스파르타의 하루
            </h2>
        ...
        <div className="relative mt-12 md:mt-20 max-w-3xl mx-auto">
          ...
          <div className="flex flex-col gap-12">
            {schedule.map((item, i) => {
              const Icon = typeof item.icon === 'string' ? (iconMap[item.icon] || Sun) : item.icon
              return (
              <motion.div
                key={i}
              ...
              >
                ...
                <div className={`flex-1 md:w-1/2 ${i % 2 === 0 ? 'md:pr-16 md:text-right' : 'md:pl-16 md:text-left'} ml-12 md:ml-0`}>
                  <p className="text-[#007AFF] text-[11px] md:text-xs font-semibold tracking-[0.2em] uppercase mb-2 opacity-80">{item.time}</p>
                  <h3 className="text-2xl md:text-3xl font-semibold text-[#1D1D1F] tracking-tight mb-2">{item.label}</h3>
                ...
                <div className="absolute left-4 md:left-1/2 -translate-x-1/2 z-10">
                  <div className="w-12 h-12 rounded-full bg-white border-4 border-[#F5F5F7] flex items-center justify-center shadow-premium-hover transition-transform duration-500 hover:scale-110">
                    <Icon size={22} className="text-[#1D1D1F]" strokeWidth={2.5} />
                  </div>
                </div>
